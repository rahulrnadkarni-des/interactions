...
export default function JoystickSVG({ onChange }: JoystickProps): ReactElement {
  ...
  const springBallScale = useSpring(1, { stiffness: 260, damping: 22 })

  const rodRef  = useRef<SVGGElement>(null)
  const ballRef = useRef<SVGGElement>(null)
  const rotVal  = useRef(0)
  const syVal   = useRef(1)
  const bsVal   = useRef(1)

  const setTransforms = () => {
    const r  = rotVal.current
    const sy = syVal.current
    const bs = bsVal.current
    rodRef.current?.setAttribute(
      'transform',
      `rotate(${r} ${PIVOT_X} ${PIVOT_Y}) translate(${PIVOT_X} ${PIVOT_Y}) scale(1 ${sy}) translate(${-PIVOT_X} ${-PIVOT_Y})`
    )
    ballRef.current?.setAttribute(
      'transform',
      `translate(${PIVOT_X} ${BALL_CY}) scale(${bs} ${bs / sy}) translate(${-PIVOT_X} ${-BALL_CY})`
    )
  }

  useMotionValueEvent(springRotate,    'change', (r)  => { rotVal.current = r;  setTransforms() })
  useMotionValueEvent(springScaleY,    'change', (sy) => { syVal.current  = sy; setTransforms() })
  useMotionValueEvent(springBallScale, 'change', (bs) => { bsVal.current  = bs; setTransforms() })
  ...
  const bind = useDrag(
    ({ offset: [ox, oy], last }) => {
      ...
      const nx    = cx / DRAG_RADIUS
      const ny    = cy / DRAG_RADIUS
      const above = Math.sqrt(nx * nx + ny * ny) > 0.3
      if (above && !wasAboveThreshold.current && 'vibrate' in navigator) navigator.vibrate(15)
      wasAboveThreshold.current = above

      const svgDragX = cx * SVG_UNITS_PER_PX
      const svgDragY = cy * SVG_UNITS_PER_PX
      const rotAngle = Math.atan2(svgDragX, ROD_LENGTH - svgDragY) * (180 / Math.PI)

      springRotate.set(rotAngle)
      springScaleY.set(1 - ny * 0.15)
      springBallScale.set(1 + ny * 0.08)
      onChange?.({ x: nx, y: ny })
    },
      ...
      >
        ...
        >
          ...
          <g>
            ...
            <path d="M72.4707 135.542C110.286 135.542 140.941 152.171 140.941 172.685C140.941 173.027 140.932 173.369 140.915 173.709V189.339C140.932 189.68 140.941 190.022 140.941 190.365C140.941 210.878 110.286 227.507 72.4707 227.507C34.6556 227.507 4.00018 210.878 4 190.365C4 189.989 4.01178 189.614 4.03223 189.241V173.808C4.01181 173.435 4 173.061 4 172.685C4.00023 152.171 34.6556 135.542 72.4707 135.542Z" fill="#11051A" />
            ...
            <ellipse cx="72.4706" cy="172.685" rx="68.4706" ry="37.1431" fill="#2C133E" />
          </g>
          ...
          <g ref={rodRef}>
            ...
            />
            <path
              d="M65.226 104.735C65.226 104.735 65.226 103.012 68.4706 103.012C71.7151 103.012 71.7151 104.735 71.7151 104.735V165.962C71.7151 165.962 71.7151 167.685 68.4706 167.685C65.226 167.685 65.226 165.962 65.226 165.962Z"
              fill="#CCE4F8"
            />
            <g ref={ballRef} className={rumbling ? 'jsk-rumbling' : undefined}>
              <circle cx="72.4706" cy="54" r="52" fill="#FFB031" stroke="black" strokeWidth="4" />
              <path d="M76.0302 4.125C101.983 5.95049 122.471 27.5826 122.471 54C122.471 81.6142 100.085 104 72.4706 104C44.8563 104 22.4706 81.6142 22.4706 54C22.4706 52.4083 22.5458 50.834 22.6913 49.2803C24.8668 70.6125 42.8854 87.2568 64.7919 87.2568C88.1652 87.2568 107.113 68.3089 107.113 44.9355C107.113 25.4534 93.9486 9.04819 76.0302 4.125Z" fill="#F39B0C" />
